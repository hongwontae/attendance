...
import CourseStudentUpdateModal from "./CourseStudentUpdateModal";
import CourseStudentAttendanceUpdateModal from "./CourseStudentAttendanceUpdateModal";
...
function CourseDetailStudent({ student, courseId }: Props) {
  const store = courseStore((cou) => cou.mode);
  const setStore = courseStore((cou) => cou.setMode);
  ...
  return (
    <>
      <div className="space-y-10">
        ...
        <div className="grid grid-cols-2 gap-20">
          ...
          <section className="space-y-6">
            <h2 className="text-gray-400 font-semibold">기본 정보</h2>
          ...
          {/* 출석 정보 */}
          <section className="space-y-6">
            <h2 className="text-gray-400 font-semibold">출석 정보</h2>

            <InfoItem label="출석률" value={`${data.attendanceRate}%`} />
        ...
        <section className="space-y-4">
          <h2 className="text-gray-400 font-semibold text-lg">메모</h2>

          <div className="rounded-2xl bg-[#95A78D] p-6 min-h-[120px] leading-relaxed shadow-sm">
            {detail.memo ?? "메모 없음"}
          </div>
        </section>
      </div>
      <AnimatePresence>
        {store === "update" && (
          <CourseStudentUpdateModal
            setStore={setStore}
          ></CourseStudentUpdateModal>
        )}
        {store === "att" && (
          <CourseStudentAttendanceUpdateModal
            setStore={setStore}
          ></CourseStudentAttendanceUpdateModal>
        )}
      </AnimatePresence>
